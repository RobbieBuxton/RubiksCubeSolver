...
\begin{minipage}{0.45\textwidth}
    ...
    \begin{tikzpicture}[node distance=2cm]
        ...
        \node (check_visited) [scale=0.5, decision, align=center] at (3.5,-4.5)
        {Have already \\ visited this \\ state?};
        \node (add_hash) [scale=0.5, process, align=center] at (1.5,-4.5) 
        {Adds state to \\ visited hash tree};
        \node (check_solved) [scale=0.5, decision, align=center] at (3.5,-6.5)
        {Is the cube \\ in a solved \\ state?};
        \node (solved) [scale=0.5, io, align=center] at (1.5, -6.5) 
        {Return the \\ solution};
        \node (expand_move) [scale=0.5, process, align=center] at (3.5,-8) 
        {Add all adjacent states \\ not already visited\\ to the queue };
        ...
        \draw [arrow] (pop) -- (check_visited);
        \draw [arrow] (check_visited) -- node[scale=0.5, yshift=0.3cm] {no} (add_hash);
        \draw [arrow] (check_visited) -- node[scale=0.5, xshift=0.5cm] {yes} (check_solved);
        \draw [arrow] (add_hash) -- (1.5,-5) -- (check_solved);
        ...
        \draw [arrow] (expand_move) -- (5,-8) -- (5,-1.5) -- (check_empty);
